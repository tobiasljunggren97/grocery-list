...
export const MiniMenuDropDown = forwardRef((props,ref) => {
    ...
    const CategoriesMenu = () => {
        ...
        <div className="mini-menu-dropdown">
            <div>{groceryList[miniMenuDroppedDown.index].item}</div>
            {categories().length > 0 ? <div className="mini-menu-categories-list">{categories().map(category => <div className="mini-menu-dropdown-item" onClick={() => handleItemCategoryChange(category)}>{category}</div>)}</div> : null}
